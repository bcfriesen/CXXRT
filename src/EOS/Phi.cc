...
double Phi(const AtomicLevel &level, const Ion &ion, const Atom &atom, const double &temperature) {
    ...
    return (double(level.g) / double(ion.continuum_state->g)) * C_I * std::pow(temperature, -3.0/2.0) * std::exp((ion.ionization_potential - level.energy) / (k_boltzmann * temperature));
}
...
double Phi_tilde(const AtomicLevel &level, const Ion &ion, const Atom &atom, const double &temperature) {
    return (double(ion.continuum_state->g) / ion.partition_function) * Phi(level, ion, atom, temperature);
}

// Mihalas defines the other Phi_tilde function in his Eq (5-16)
double Phi_tilde(const Ion &ion, const Atom &atom, const double &temperature) {

    const double C_I = 0.5 * std::pow(std::pow(h_planck, 2) / (2.0 * pi * m_electron * k_boltzmann), 3.0/2.0); // Mihalas defines this constant in his Eq (5-14)

    return (ion.partition_function / ion.next_ion->partition_function) * C_I * std::pow(temperature, -3.0/2.0) * std::exp(ion.ionization_potential / (k_boltzmann * temperature));
}
